...
@app.route("/get-test-images/<int:start>/<int:end>", methods=["GET"])
def get_validation_images(start, end):
    """Route to get test images, converts images to base64 for transmission."""
    images_base64 = []
...
def predict():
    ...
    try:
        ...
        return jsonify(
            {"prediction": int(predicted_class), "confidence": float(confidence)}
        )
    except KeyError:
